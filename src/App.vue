<template>
  <div id="app" :style="{ background: getTheme.bodyBackgroundColor }">
    <div class="home_audio_inint" v-show="homeAudioInint" @click="changePlay">
      <div class="fensug">
        <div class="wavenum">
          <b id="denfenjs">{{ waveHeight }}%</b><tt>{{ waveNews }}</tt>
        </div>
        <div class="waven">
    ...
    <div class="home_audio_inint" v-if="videoBoo" @click="playVideo()">
      <iframe
        class="home_video"
        frameborder="0"
        src="https://v.qq.com/txp/iframe/player.html?vid=o3029j2jdrg"
    ...
    <div class="home" :style="{ opacity: loadSpecialEffects, 'background-color': getTheme.frameColor }">
      <div class="home_audio" @click="changePlay" id="change_play" :title="musicName" v-show="!homeAudioInint">
        ...
        <div
          id="home_audio_div"
          class="home_audio_div"
          :title="musicName"
          :style="{ color: getTheme.other.audioColor, 'background-color': getTheme.other.audioBackgroundColor }"
        >
          {{ audioExpress }}
        </div>
      </div>
      <img
        class="home_vlog"
        v-show="!homeAudioInint"
        @click="playVideo()"
        title="这世界上总有一份美好能被你喜欢"
        src="../static/vlog_maldway.png"
      />
      ...
      <div class="home_tracing">
        <div class="home_egg" id="home_egg" :style="{ 'background-color': getTheme.backgroundColor }">
          ...
          <transition name="fade">
            <router-view :name="getTheme.name"> </router-view>
          </transition>
        </div>
      </div>
      <div class="home_egg_botom">
        <div
          class="home_egg_botom_button"
          @click="backOf()"
          :style="{ 'border-color': getTheme.other.buttonBorderColor }"
        >
          <div
            class="home_egg_botom_button_square"
            :style="{ 'background-color': getTheme.other.buttonCoreColor }"
          ></div>
        </div>
      </div>
    </div>
...
.home_egg::-webkit-scrollbar {
  /* height: 10px; */
  width: 8px;
}

/* 滚动条的滑轨背景颜色 */
.home_egg::-webkit-scrollbar-track {
  background-color: #fdfdfd;
}

/* 滑块颜色 */
.home_egg::-webkit-scrollbar-thumb {
  background-color: rgba(63, 63, 63, 0.2);
  border-radius: 5px;
}

/* 滑轨两头的监听按钮颜色 */
.home_egg::-webkit-scrollbar-button {
  height: 10px;
  background-color: #fdfdfd;
}
